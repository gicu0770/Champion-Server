...
bool Map::loadMap(const std::string& identifier, bool loadHouses, const Position& pos, bool unload, bool lua, int8_t instance)
{
	...
	if (!lua) {
		if (!IOMap::loadSpawns(this, instance)) {
...
bool Map::respawnDungeon(const std::string& identifier, DungeonInstance* instance, const Position& pos, uint8_t players)
{
	if (instance->getChunkId() >= chunkSpawns.size())
		chunkSpawns.resize(static_cast<size_t>(instance->getChunkId()) + 1);

	if (chunkSpawns[instance->getChunkId()].isStarted()) {
		chunkSpawns[instance->getChunkId()].clear();
	}

	if (!IOMap::loadChunkSpawns(this, identifier, pos, instance->getChunkId())) {
		std::cout << "[Warning - Map::respawnDungeon] Failed to load chunk spawn data." << std::endl;
	}

	chunkSpawns[instance->getChunkId()].startup(instance, players);
	return true;
}
...
bool Map::save()
{
	...
	// Use async version - returns immediately, saves in background
	IOMapSerialize::saveHouseItemsAsync();
	return true;
}
...
const Tile* Map::canWalkTo(const Creature& creature, const Position& pos) const
{
	...
	if (!tile || tile->queryAdd(0, creature, 1, FLAG_PATHFINDING | FLAG_IGNOREFIELDDAMAGE | FLAG_IGNOREBLOCKCREATURE) != RETURNVALUE_NOERROR) {
		return nullptr;
	}
	return tile;
}
...
bool Map::getPathMatching(const Creature& creature, const Position& targetPos, std::vector<Direction>& dirList, const FrozenPathingConditionCall& pathCondition, const FindPathParams& fpp) const
{
	...
	do {
		...
		for (uint_fast32_t i = 0; i < dirCount; ++i) {
			...
			const int_fast32_t cost = AStarNodes::getMapWalkCost(n, pos, fpp.ignoreDiagonalCost);
...
inline int_fast32_t AStarNodes::getMapWalkCost(AStarNode* node, const Position& neighborPos, const bool ignoreDiagonalCost /*false*/)
{
	if (ignoreDiagonalCost) {
