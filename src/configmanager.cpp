...
int64_t getGlobalNumber(lua_State* L, const char* identifier, const int64_t defaultValue = 0)
{
	lua_getglobal(L, identifier);
	if (!lua_isnumber(L, -1)) {
		lua_pop(L, 1);
...
bool ConfigManager::load()
{
	...
	boolean[WHITELIST_ENABLED] = getGlobalBoolean(L, "whitelistEnabled", false);
	...
	string[CAMS_DIR] = getGlobalString(L, "camsDir", "");
	#if GAME_FEATURE_STORE > 0
	string[STORE_URL] = getGlobalString(L, "storeUrl", "http://static.tibia.com/images/store/");
	#endif

	integer[CLIENT_UPDATE_VERSION] = getGlobalNumber(L, "clientUpdateVersrion");
	...
	integer[DEATH_LOSE_PERCENT_RED_SKULL] = getGlobalNumber(L, "deathLosePercentRedSkull", -1);
	integer[DEATH_LOSE_PERCENT_BLACK_SKULL] = getGlobalNumber(L, "deathLosePercentBlackSkull", -1);
	...
	integer[INSTANCE_TYPE] = getGlobalNumber(L, "instanceType", 0); 
...
int64_t ConfigManager::getNumber(integer_config_t what) const
{
...
void ConfigManager::setNumber(integer_config_t what, int64_t value)
{
	if (what >= LAST_INTEGER_CONFIG) {
		std::cout << "[Warning - ConfigManager::getString] Accessing invalid index: " << what << std::endl;
		return;
