...
enum ServerType_t : uint8_t {
	MAIN_SERVER = 0,
	DUNGEON_INSTANCE = 1,
};

enum DungeonCompleteType : uint8_t {
	DUNGEONTYPE_KILL_PERCENT = 0,
	DUNGEONTYPE_OBJECTIVES = 1,
};
...
enum itemAttrTypes : uint32_t {
	...
	ITEM_ATTRIBUTE_AUTOOPEN = 1 << 24,
	ITEM_ATTRIBUTE_REALUID = 1 << 25,
	ITEM_ATTRIBUTE_MAXSIZE = 1 << 26,
...
enum CombatType_t : uint16_t {
	...
	COMBAT_PHYSICAL_PROC_DAMAGE = 1 << 12,
	COMBAT_ELEMENTAL_PROC_DAMAGE = 1 << 13,
	COMBAT_PHYSICAL_DOT = 1 << 14,
	COMBAT_ELEMENTAL_DOT = 1 << 15,
...
enum CombatParam_t {
	...
	COMBAT_PARAM_TYPEOB,
	COMBAT_PARAM_BOTTOMEFFECT,
	COMBAT_PARAM_DAMAGE,
	COMBAT_PARAM_DAMAGE2,
	COMBAT_PARAM_TYPE2,
	COMBAT_PARAM_CENTEREFFECT,
	COMBAT_PARAM_OFFSETXEFFECT,
	COMBAT_PARAM_OFFSETYEFFECT,
	COMBAT_PARAM_SAVEPOS,
	COMBAT_PARAM_COLOR,
};
...
enum ConditionParam_t {
	...
	CONDITION_PARAM_STAT_MAXENERGYSHIELD = 54,
	CONDITION_PARAM_STAT_MAXENERGYSHIELDPERCENT = 55,
	CONDITION_PARAM_ENERGYGAIN = 56,
	CONDITION_PARAM_ENERGYTICKS = 57,
	CONDITION_PARAM_ATTACKSPEED = 58,
	CONDITION_PARAM_MAXATTACKSPEED = 59,
	CONDITION_PARAM_EXTRARANGEMELEE = 60,
	CONDITION_PARAM_EXTRARANGERANGE = 61,
};
...
enum stats_t {
	...
	STAT_MAXMANAPOINTS,
	STAT_MAGICPOINTS,
	STAT_MAXENERGYSHIELD,

	STAT_MAXHITPOINTSPERCENT,
	STAT_MAXMANAPOINTSPERCENT,
	STAT_MAGICPOINTSPERCENT,
	STAT_MAXENERGYSHIELDPERCENT,
	STAT_ATTACKSPEED,
	STAT_MAXATTACKSPEED,
	STAT_EXTRARANGEMELEE,
	STAT_EXTRARANGERANGE,

	STAT_FIRST = STAT_MAXHITPOINTS,
	STAT_LAST = STAT_EXTRARANGERANGE
};
...
enum ConditionType_t : uint64_t {
    ...
    CONDITION_EXHAUST_WEAPON = 1ULL << 12,
    ...
    CONDITION_FEAR = 1ULL << 23,
    CONDITION_ROOT = 1ULL << 24,
...
enum ReturnValue {
	...
	RETURNVALUE_CONTAINERONLYSPELLRUNES,
	RETURNVALUE_CONTAINERONLYONE,
	...
	RETURNVALUE_SPAWNBLOCKINGTHISCREATURE,
...
struct PlayerInventorySellItem 
{
	uint8_t rarity = 0;
	uint32_t itemId = 0;
};
...
struct Outfit_t {
	...
	uint16_t lookHealthBar = 1;
	uint16_t lookManaBar = 1;
...
{
	HighscoreCharacter(std::string name, uint64_t points, uint32_t id, uint32_t rank, uint16_t level, uint8_t vocation, uint8_t online, Outfit_t outfit) :
		name(std::move(name)), points(points), id(id), rank(rank), level(level), vocation(vocation), online(online), outfit(outfit) {}
...
struct MarketOffer
{
	MarketOffer(uint64_t marketId, uint32_t clientId, std::string seller, uint16_t amount, uint64_t created, double price, uint16_t currency, std::string name, uint16_t rarity) :
		marketId(marketId), clientId(clientId), seller(std::move(seller)), amount(amount), created(created), price(price), currency(currency), name(std::move(name)), rarity(rarity) {}
...
struct CombatDamage
{
	...
	bool leeched;
	int16_t critChance;
	int16_t critMultiplier;
	uint32_t uid; // item uid for lua chace spell
	bool gambler;
...
enum CharacterStats_t
{
	...
	CHARSTAT_WISDOM,
	CHARSTAT_PERCEPTION,
	CHARSTAT_CRITICAL_DAMAGE,
	CHARSTAT_REGEN,
	CHARSTAT_LIFE,
	CHARSTAT_MOVEMENT_SPEED,
	CHARSTAT_ONE,
	CHARSTAT_TWO,
	CHARSTAT_HPHIT,
	CHARSTAT_ESHIT,
...
constexpr int16_t ATTRIBUTE_EMPTY = INT16_MIN;

enum ServerToServerOpcodes : uint8_t {
	STS_OPCODE_STATUS = 1,
	STS_OPCODE_GET_ONLINE_COUNT = 2,
	STS_OPCODE_GET_ONLINE_PLAYERS = 3,
	STS_OPCODE_CLOSE_INSTANCE = 4,
};
