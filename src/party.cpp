...
void Party::disband()
{
	...
	currentLeader->sendPartyDisband();
...
bool Party::getPartyInfo(Player* player)
{
	for (Player* member : memberList) {
		if (player != member) {
			player->sendPartyJoin(member);
			...
			player->sendPartyPlayerPos(member, member->getPosition());
		}
	}

	if (player != leader) {
		player->sendPartyJoin(leader);
		player->sendPartyCreatureHealth(leader, std::ceil((static_cast<double>(leader->getHealth()) / std::max<int64_t>(leader->getMaxHealth(), 1)) * 100));
		player->sendPartyPlayerMana(leader, std::ceil((static_cast<double>(leader->getMana()) / std::max<int64_t>(leader->getMaxMana(), 1)) * 100));
		player->sendPartyPlayerPos(leader, leader->getPosition());
	}
	return true;
}
...
bool Party::leaveParty(Player* player)
{
	...
	player->sendPartyDisband();
	leader->sendPartyLeave(player);
...
bool Party::kickPartyMember(Player* player)
{
	...
	std::stringExtended ss(player->getName().length() + static_cast<size_t>(32));
	ss << player->getName() << " was kicked from the party.";
...
bool Party::passPartyLeadership(Player* player)
{
	...
	for (Player* member : memberList) {
		...
		member->sendPartyUpdateShield(oldLeader);
		member->sendPartyUpdateShield(leader);
	}
	...
	leader->sendPartyUpdateShield(oldLeader);
	leader->sendPartyUpdateShield(leader);
...
bool Party::invitePlayer(Player& player)
{
	...
	player.sendPartyInvite(leader->getName(), leader->getID());
	return true;
}
...
void Party::updateAllPartyIcons()
{
	for (Player* member : memberList) {
		for (Player* otherMember : memberList) {
			...
			member->sendPartyUpdateShield(otherMember);
		}

		member->sendCreatureShield(leader);
		leader->sendCreatureShield(member);
		member->sendPartyUpdateShield(leader);
		leader->sendPartyUpdateShield(member);
	}
	leader->sendCreatureShield(leader);
	leader->sendPartyUpdateShield(leader);
}
...
void Party::shareExperience(uint64_t experience, Creature* source/* = nullptr*/)
{
	...
	g_events->eventPlayerOnGainExperience(leader, source, shareExperience, shareExperience);
...
void Party::updatePlayerPosition(const Player* player, const Position& pos)
{
	if (!player || !leader) {
		return;
	}
	...
	for (Player* member : memberList) {
		if (member) {
			member->sendPartyPlayerPos(player, pos);
		}
	}

	leader->sendPartyPlayerPos(player, pos);
}
