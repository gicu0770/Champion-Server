...
bool Weapon::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	int32_t damageModifier = playerWeaponCheck(player, target, item->getShootRange() + player->getVarStats(STAT_EXTRARANGEMELEE));
	if (damageModifier == 0) {
		return false;
	}
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	int32_t maxDamage = static_cast<int32_t>(std::round( (float)attackFactor ));
	...
	damage.primary.value = -normal_random(maxDamage, maxDamage);
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	if (scripted) {
		...
		// paladyn holy
		// if (player->getVocationId() == 17 || player->getVocationId() == 18 || player->getVocationId() == 19 || player->getVocationId() == 20) {
		//	damage.primary.type = COMBAT_HOLYDAMAGE;
		//} else if (player->getVocationId() == 21 || player->getVocationId() == 22 || player->getVocationId() == 23 || player->getVocationId() == 24) {
...
void Weapon::onUsedWeapon(Player* player, Item* item, Tile* destTile) const
{
	...
	int32_t maxValue = static_cast<int32_t>(std::round( (float)attackFactor ));
	player->addStorageValue(6786789, maxValue);
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	int32_t maxValue = static_cast<int32_t>(static_cast<int32_t>(std::round( (float)attackFactor )) * player->getVocation()->meleeDamageMultiplier);
	if (maxDamage) {
		return -maxValue;
	}

	return -normal_random(maxValue, maxValue);
}
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if (it.weaponType == WEAPON_AMMO) {
		...
		if (mainWeapon) {
			damageModifier = mainWeapon->playerWeaponCheck(player, target, mainWeaponItem->getShootRange() + player->getVarStats(STAT_EXTRARANGERANGE));
		} else {
			damageModifier = playerWeaponCheck(player, target, mainWeaponItem->getShootRange() + player->getVarStats(STAT_EXTRARANGERANGE));
		}
	} else {
		damageModifier = playerWeaponCheck(player, target, item->getShootRange() + player->getVarStats(STAT_EXTRARANGERANGE));
	}
	...
	chance = 100;
	if (chance >= uniform_random(100, 100)) {
...
bool WeaponWand::getSkillType(const Player* player, const Item*, skills_t& skill, uint32_t& skillpoint) const
{
	skill = SKILL_FISHING;
