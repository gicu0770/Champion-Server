...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("power"))) {
		std::string tmpStr = asLowerCaseString(attr.as_string());
		power = tmpStr;
	}

	if ((attr = node.attribute("image"))) {
		std::string tmpStr = asLowerCaseString(attr.as_string());
		image = tmpStr;
	}
	...
	if ((attr = node.attribute("mana"))) {
		mana = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = node.attribute("manapercent"))) {
		manaPercent = pugi::cast<uint64_t>(attr.value());
	}
		...
		if (!player->hasFlag(PlayerFlag_HasNoExhaustion)) {
			...
			if (groupCooldown > 0) {
				...
				if (finalgroupCooldown < 300) {
					finalgroupCooldown = 300;
				}
				Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, finalgroupCooldown, 0, false, group);
				player->addCondition(condition);
			}
...
void Spell::postCastSpell(Player* player, uint64_t manaCost, uint32_t soulCost)
{
	if (manaCost > 0) {
...
uint64_t Spell::getManaCost(const Player* player) const
{
	uint64_t manaCost = mana;
	if (manaPercent != 0) {
