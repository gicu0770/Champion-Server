...
struct Outfit {
	Outfit(std::string name, uint16_t lookType, bool free, bool unlocked, uint16_t vocation, uint8_t rarity) :
		name(std::move(name)), lookType(lookType), free(free), unlocked(unlocked), vocation(vocation), rarity(rarity) {}
