...
class Spawn
{
	public:
		...
		void addPreSpawnBlocker(Creature* creature, Monster* monster);
	...
	private:
		...
		CreatureHashSet preSpawnBlockers;
		CreatureHashSet spawnBlockers;
...
class Spawns
{
	public:
		...
		bool loadFromXml(const std::string& filename, int8_t instance = 1);
		void startup();
		void clear();
		void setLoaded() {
			loaded = true;
