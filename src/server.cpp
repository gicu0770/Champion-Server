...
void ServiceManager::stop()
{
	...
	if (!death_timer) {
		death_timer = std::make_unique<boost::asio::deadline_timer>(io_context);
	}
...
void ServicePort::onAccept(Connection_ptr connection, const boost::system::error_code& error)
{
	if (!error) {
		...
		if (!pendingStart) {
			...
			pendingStart = true;
			if (!deadline_timer) {
				deadline_timer = std::make_unique<boost::asio::deadline_timer>(io_context);
			}
...
void ServicePort::open(uint16_t port)
{
	...
	try {
		if (g_config.getBoolean(ConfigManager::BIND_ONLY_GLOBAL_ADDRESS)) {
			...
			try {
				addr = boost::asio::ip::make_address_v4(g_config.getString(ConfigManager::IP));
			} catch (const std::exception& ex) {
				std::cout << "[ServicePort::open] Invalid IP address: " << g_config.getString(ConfigManager::IP) << " - " << ex.what() << std::endl;
				return;
			}
			acceptor.reset(new boost::asio::ip::tcp::acceptor(io_context, boost::asio::ip::tcp::endpoint(
			            boost::asio::ip::address(addr), serverPort)));
		} else {
			acceptor.reset(new boost::asio::ip::tcp::acceptor(io_context, boost::asio::ip::tcp::endpoint(
