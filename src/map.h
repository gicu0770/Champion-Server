...
static constexpr int32_t MAP_CHANNELS = 1;
static constexpr int32_t MAP_MAX_LAYERS = 16 * MAP_CHANNELS;
...
class AStarNodes
{
	public:
		...
		static inline int_fast32_t getMapWalkCost(AStarNode* node, const Position& neighborPos, const bool ignoreDiagonalCost = false);
...
class Map
{
	public:
		...
		/**
		  ...
		  */
		bool loadMap(const std::string& identifier, bool loadHouses, const Position& pos = Position(), bool unload = false, bool lua = false, int8_t instance = 1);
		void setLoaded();
		bool loadDungeon(const std::string& identifier, const Position& pos);
		bool respawnDungeon(const std::string& identifier, DungeonInstance* instance, const Position& pos, uint8_t players);
