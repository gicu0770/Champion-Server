...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if ((attr = node.attribute("min"))) {
		sb.minCombatValue = pugi::cast<int64_t>(attr.value());
	}

	if ((attr = node.attribute("max"))) {
		sb.maxCombatValue = pugi::cast<int64_t>(attr.value());

		//normalize values
		if (std::abs(sb.minCombatValue) < std::abs(sb.maxCombatValue)) {
//			int32_t value = sb.maxCombatValue;
			int64_t value = (sb.minCombatValue + sb.maxCombatValue) / 2;
			sb.maxCombatValue = value;
			sb.minCombatValue = value;
		}
	}
	if ((attr = node.attribute("name"))) {
        sb.nameSpell = attr.as_string();
    }
	if ((attr = node.attribute("radius"))) {
		sb.radius = pugi::cast<int32_t>(attr.value());
	}
	if ((attr = node.attribute("length"))) {
		sb.length = pugi::cast<int32_t>(attr.value());
	}
	if ((attr = node.attribute("spread"))) {
		sb.spread = pugi::cast<int32_t>(attr.value());
	}
	if ((attr = node.attribute("target"))) {
		sb.target = pugi::cast<int32_t>(attr.value());
	}
	...
	if (isScripted) {
		...
		if (!tfs_strcmp(tmpName.c_str(), "melee")) {
			...
			if ((attackAttribute = node.attribute("attack")) && (skillAttribute = node.attribute("skill"))) {
				sb.minCombatValue = 0;
				sb.maxCombatValue = -Weapons::getMaxMeleeDamage(pugi::cast<int64_t>(skillAttribute.value()), pugi::cast<int64_t>(attackAttribute.value()));
			}
			...
			if ((attr = node.attribute("fire"))) {
				...
				minDamage = pugi::cast<int64_t>(attr.value());
				maxDamage = minDamage;
				tickInterval = 9000;
			...
			int64_t minDamage = std::abs(sb.minCombatValue);
			int64_t maxDamage = std::abs(sb.maxCombatValue);
			int64_t startDamage = 0;

			if ((attr = node.attribute("start"))) {
				int64_t value = std::abs(pugi::cast<int64_t>(attr.value()));
				if (value <= minDamage) {
					startDamage = value;
				}
			}
				...
				if (strcasecmp(value, "shooteffect") == 0) {
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (shoot != CONST_ANI_NONE) {
							...
							combat->setParam(COMBAT_PARAM_DISTANCEEFFECT, pugi::cast<uint16_t>(attr.value()));
						}
					}
					if ((attr = attributeNode.attribute("effect"))) {
						combat->setParam(COMBAT_PARAM_TYPEOB, attr.as_bool());
					}
					...
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (effect != CONST_ME_NONE) {
							...
							combat->setParam(COMBAT_PARAM_EFFECT, pugi::cast<uint16_t>(attr.value()));
						}
					}
				} else {
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (std::abs(sb.minCombatValue) > std::abs(sb.maxCombatValue)) {
		int64_t value = sb.maxCombatValue;
	...
	if (spell->isScripted) {
		...
		if (!tfs_strcmp(tmpName.c_str(), "melee")) {
			...
			int64_t minDamage = std::abs(spell->conditionMinDamage);
			int64_t maxDamage = std::abs(spell->conditionMaxDamage);
			int64_t startDamage = 0;

			if (spell->conditionStartDamage != 0) {
				int64_t value = std::abs(spell->conditionStartDamage);
				if (value <= minDamage) {
					startDamage = value;
				}
			}
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("zone"))) {
		mType->info.zoneId = pugi::cast<uint32_t>(attr.value());;
	}

	if ((attr = monsterNode.attribute("bestiaryId"))) {
		mType->info.bestiaryId = pugi::cast<uint32_t>(attr.value());;
	}
	...
	if ((attr = monsterNode.attribute("race"))) {
		...
		} else if (!tfs_strcmp(tmpStrValue.c_str(), "boss") || tmpInt == 6) {
			mType->info.race = RACE_BOSS;
		} else {
	...
	if ((attr = monsterNode.attribute("manacost"))) {
		mType->info.manaCost = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("tier"))) {
		mType->info.tier = pugi::cast<uint32_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("items"))) {
		mType->info.items = attr.as_string();
	}
	...
	pugi::xml_node titleNode = monsterNode.child("title");
	if (titleNode) {
		if ((attr = titleNode.attribute("text"))) {
			mType->info.titleText = attr.as_string();
		}

		if ((attr = titleNode.attribute("font"))) {
			mType->info.titleFont = attr.as_string();
		}

		if ((attr = titleNode.attribute("color"))) {
			mType->info.titleColor = attr.as_string();
		}
	}
	...
	if ((node = monsterNode.child("look"))) {
		...
		if ((attr = node.attribute("outline"))) {
			mType->info.outfit.lookOutline = attr.as_string();
		}
		if ((attr = node.attribute("healthbar"))) {
			mType->info.outfit.lookHealthBar = pugi::cast<uint16_t>(attr.value());
		}

		if ((attr = node.attribute("manabar"))) {
			mType->info.outfit.lookManaBar = pugi::cast<uint16_t>(attr.value());
		}
	...
	if (mType->info.zoneId != 0) {
		if (!reloading && zoneMonsters[mType->info.zoneId][monsterName]) {
			std::cout << "[Warning - Monsters::loadMonster] Duplicate Zone Id " << file << std::endl;
		}
		zoneMonsters[mType->info.zoneId][monsterName] = mType;
	}

	if (mType->info.bestiaryId != 0) {
		if (!reloading && bestiaryMonsters[mType->info.bestiaryId]) {
			std::cout << "[Warning - Monsters::loadMonster] Duplicate Bestiary Id: " << file << std::endl;
		}
		bestiaryMonsters[mType->info.bestiaryId] = mType;
	}
...
MonsterType* Monsters::getMonsterTypeByBestiaryId(uint32_t id)
{
	auto it = bestiaryMonsters.find(id);
...
std::map<std::string, MonsterType*>* Monsters::getMonstersTypeByZoneId(uint32_t id)
{
	auto it = zoneMonsters.find(id);
	if (it == zoneMonsters.end()) {
		return nullptr;
	}
	return &it->second;
}
