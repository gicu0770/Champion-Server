...
bool ConditionAttributes::setParam(ConditionParam_t param, int64_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_STAT_MAXENERGYSHIELD: {
			stats[STAT_MAXENERGYSHIELD] = value;
			return true;
		}
		...
		case CONDITION_PARAM_STAT_MAXHITPOINTSPERCENT: {
			stats[STAT_MAXHITPOINTSPERCENT] = value;
			return true;
		}

		case CONDITION_PARAM_STAT_MAXENERGYSHIELDPERCENT: {
			stats[STAT_MAXENERGYSHIELDPERCENT] = value;
			return true;
		}

		case CONDITION_PARAM_STAT_MAXMANAPOINTSPERCENT: {
			stats[STAT_MAXMANAPOINTSPERCENT] = value;
			return true;
		}

		case CONDITION_PARAM_STAT_MAGICPOINTSPERCENT: {
			stats[STAT_MAGICPOINTSPERCENT] = value;
			return true;
		}

		case CONDITION_PARAM_ATTACKSPEED: {
			stats[STAT_ATTACKSPEED] = value;
			return true;
		}

		case CONDITION_PARAM_MAXATTACKSPEED: {
			stats[STAT_MAXATTACKSPEED] = value;
			return true;
		}

		case CONDITION_PARAM_EXTRARANGEMELEE: {
			stats[STAT_EXTRARANGEMELEE] = value;
			return true;
		}

		case CONDITION_PARAM_EXTRARANGERANGE: {
			stats[STAT_EXTRARANGERANGE] = value;
			return true;
		}
...
bool ConditionRegeneration::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	if (attr == CONDITIONATTR_HEALTHTICKS) {
		return propStream.read<uint64_t>(healthTicks);
	} else if (attr == CONDITIONATTR_HEALTHGAIN) {
		return propStream.read<uint64_t>(healthGain);
	} else if (attr == CONDITIONATTR_MANATICKS) {
		return propStream.read<uint64_t>(manaTicks);
	} else if (attr == CONDITIONATTR_MANAGAIN) {
		return propStream.read<uint64_t>(manaGain);
	}
	return Condition::unserializeProp(attr, propStream);
}
...
void ConditionRegeneration::serialize(PropWriteStream& propWriteStream)
{
	...
	propWriteStream.write<uint64_t>(healthTicks);

	propWriteStream.write<uint8_t>(CONDITIONATTR_HEALTHGAIN);
	propWriteStream.write<uint64_t>(healthGain);

	propWriteStream.write<uint8_t>(CONDITIONATTR_MANATICKS);
	propWriteStream.write<uint64_t>(manaTicks);

	propWriteStream.write<uint8_t>(CONDITIONATTR_MANAGAIN);
	propWriteStream.write<uint64_t>(manaGain);
}
...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	// if (creature->canRegenerateShield() && internalEnergyTicks >= energyTicks) {
	// 	internalEnergyTicks = 0;
	// 	creature->changeEnergyShield(energyGain);
	// }
...
bool ConditionDamage::doDamage(Creature* creature, int64_t healthChange)
{
	...
	if (field && creature->getPlayer() && attacker && attacker->getPlayer()) {
		damage.primary.value = static_cast<int64_t>(std::round(damage.primary.value / 2.));
	}
