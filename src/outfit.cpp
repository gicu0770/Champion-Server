...
bool Outfits::loadFromXml()
{
	...
	for (auto outfitNode : doc.child("outfits").children()) {
		...
		pugi::xml_attribute vocationOutfit = outfitNode.attribute("vocation");
		outfits[type].emplace_back( 
			outfitNode.attribute("name").as_string(),
			pugi::cast<uint16_t>(lookTypeAttribute.value()),
			outfitNode.attribute("free").as_bool(false),
			outfitNode.attribute("unlocked").as_bool(true),
			pugi::cast<uint16_t>(vocationOutfit.value()),
			static_cast<uint8_t>(pugi::cast<uint16_t>(outfitNode.attribute("rarity").value()))
		);
	}
