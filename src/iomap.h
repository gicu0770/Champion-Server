...
class IOMap
{
	...
	public:
		bool loadMap(Map* map, const std::string& fileName, const Position& pos = Position(), bool unload = false, bool showProgressBar = false);
		
		/* Load the spawns
		 * \param map pointer to the Map class
		 * \returns Returns true if the spawns were loaded successfully
		 */
		static bool loadSpawns(Map* map, int8_t instance = 0) {
			...
			return map->spawns.loadFromXml(map->spawnfile, instance);
		}

		static void setLoaded(Map* map) {
			map->spawns.setLoaded();
		}

		/* Load the spawns for chunk
		 * \param map pointer to the Map class
		 * \returns Returns true if the spawns were loaded successfully
		 */
		static bool loadChunkSpawns(Map* map, const std::string& mapName, const Position& pos, uint16_t instance) {
	...
	private:
		...
		bool parseTileArea(OTB::Loader& loader, const OTB::Node& tileAreaNode, Map& map, const Position& pos, bool unload, bool _legacy);
