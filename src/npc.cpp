...
bool Npc::loadFromXml()
{
	...
	pugi::xml_node titleNode = npcNode.child("title");
	if (titleNode) {
		if ((attr = titleNode.attribute("text"))) {
			titleText = attr.as_string();
		}

		if ((attr = titleNode.attribute("font"))) {
			titleFont = attr.as_string();
		}

		if ((attr = titleNode.attribute("color"))) {
			titleColor = attr.as_string();
		}
	}
	...
	if (lookNode) {
		...
		if ((attr = lookNode.attribute("outline"))) {
			defaultOutfit.lookOutline = attr.as_string();
		}
		if ((attr = lookNode.attribute("healthbar"))) {
			defaultOutfit.lookHealthBar = pugi::cast<uint16_t>(attr.value());
		}
		if ((attr = lookNode.attribute("manabar"))) {
			defaultOutfit.lookManaBar = pugi::cast<uint16_t>(attr.value());
		}
...
void Npc::onPlayerTrade(Player* player, int32_t callback, uint16_t itemId, uint8_t count,
                        uint8_t amount, uint64_t realUID, bool ignore/* = false*/, bool inBackpacks/* = false*/)
{
	if (npcEventHandler) {
		npcEventHandler->onPlayerTrade(player, callback, itemId, count, amount, realUID, ignore, inBackpacks);
	}
}

void Npc::onPlayerSellMultiple(Player* player, int32_t callback, const std::vector<uint64_t> items)
{
	if (npcEventHandler) {
		npcEventHandler->onPlayerSellMultiple(player, callback, items);
	}
}
	...
	npc(npc), scriptInterface(npc->getScriptInterface())
{
	...
	if (!loaded) {
		...
		playerSellMultipleEvent = scriptInterface->getEvent("onPlayerSellMultiple");
...
void NpcEventsHandler::onPlayerSellMultiple(Player* player, int32_t callback, const std::vector<uint64_t> items)
{
	if (playerSellMultipleEvent == -1) {
		return;
	}

	//onSellMultiple(player, items)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - NpcScript::onPlayerSellMultiple] Call stack overflow" << std::endl;
		return;
	}
	...
	ScriptEnvironment* env = scriptInterface->getScriptEnv();
	env->setScriptId(playerSellMultipleEvent, scriptInterface);
	env->setNpc(npc);

	lua_State* L = scriptInterface->getLuaState();
	scriptInterface->pushFunction(playerSellMultipleEvent);
...
void NpcEventsHandler::onPlayerTrade(Player* player, int32_t callback, uint16_t itemId,
                              uint8_t count, uint8_t amount, uint64_t realUID, bool ignore, bool inBackpacks)
{
	if (callback == -1) {
		return;
	}
