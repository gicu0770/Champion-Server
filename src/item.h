...
class ItemAttributes
{
	public:
		...
		void setMaxSize(uint32_t n) {
			setIntAttr(ITEM_ATTRIBUTE_MAXSIZE, n);
		}

		uint32_t getMaxSize() const {
			return static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_MAXSIZE));
		}
	...
	private:
		...
		void setCustomAttribute(int64_t key, std::string& value) {
			std::string tmp = boost::lexical_cast<std::string>(key);
			setCustomAttribute(tmp, value);
		}
	...
	public:
		static bool isIntAttrType(itemAttrTypes type) {
			...
			checkTypes |= ITEM_ATTRIBUTE_AUTOOPEN;
			checkTypes |= ITEM_ATTRIBUTE_REALUID;
			checkTypes |= ITEM_ATTRIBUTE_MAXSIZE;
...
class Item : virtual public Thing
{
	public:
		...
		static Item* CreateItem(const uint16_t type, uint16_t count = 0, bool loaded = false);
		...
		bool isMoveable() const {
			...
			if (id == ITEM_STORE_INBOX || id == ITEM_RELICT_BOX) {
				return false;
			}
			#endif
			return items[id].moveable;
		}
		bool isRelictBox() const {
			return id == ITEM_RELICT_BOX;
		}
		...
		bool isSupportRune() const {
			return items[id].isSupportRune();
		}
		...
		uint16_t getSpellGemId() const {
			return items[id].spellGem;
		}
		...
		bool isDepot() const {
			return items[id].type == ITEM_TYPE_DEPOT;
		}

		bool isExtendedContainer() const {
			return items[id].isExtendedContainer();
		}

		uint16_t getLootIndex() const {
			return items[id].lootIndex;
		}

		uint16_t getPotionId() const {
			return items[id].potionId;
		}
		...
		void setUniqueId(uint16_t n);
		void setRealUID(uint64_t uid) {
			realUId = uid;
		}

		uint64_t getRealUID() const {
			return realUId;
		}
		void setMaxSize(uint32_t n);		
		...
		std::string getSpellName() const {
			return items[id].spellName;
		}

		std::string getPotionName() const {
			return items[id].potionName;
		}
		...
		void updateSelf() {
			getParent()->updateThing(this, getID(), getItemCount());
		}
