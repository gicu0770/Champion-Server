...
class ConfigManager
{
	public:
		...
		enum integer_config_t {
			...
			PROTECTION_LEVEL,
			DEATH_LOSE_PERCENT,
			DEATH_LOSE_PERCENT_RED_SKULL,
			DEATH_LOSE_PERCENT_BLACK_SKULL,
			STATUSQUERY_TIMEOUT,
			FRAG_TIME,
			WHITE_SKULL_TIME,
			GAME_PORT,
		...
		int64_t getNumber(integer_config_t what) const;
		bool getBoolean(boolean_config_t what) const;
		float getFloat(floating_config_t what) const;

		void setString(string_config_t what, const std::string value);
		void setNumber(integer_config_t what, int64_t value);

	private:
		std::string string[LAST_STRING_CONFIG] = {};
		int64_t integer[LAST_INTEGER_CONFIG] = {};
