...
enum CreatureEventType_t {
	...
	CREATURE_EVENT_ENDSCRIPTS_REGEN,
	CREATURE_EVENT_MINUSREGENHP,
	CREATURE_EVENT_HEALTHDRAIN,
...
class CreatureEvent final : public Event
{
	public:
		...
		void executeOnHealthDrain(Creature* creature, Creature* attacker, int64_t damage);
		void executeManaChange(Creature* creature, Creature* attacker, CombatDamage& damage);
		void executeEndScriptsRegen(Player* player, bool health);
		void executeMinusRegenHP(Player* player, int32_t value);
		void executeExtendedOpcode(Player* player, uint8_t opcode, const std::string& buffer);
		bool executeOnReconnect(Player* player) const;
		//
...
class CreatureEvents final : public BaseEvents
{
	public:
		...
		bool playerReconnect(Player* player) const;
	...
	private:
		...
		std::vector<CreatureEvent> reconnectEvents;
