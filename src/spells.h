...
class Spell : public BaseSpell
{
	public:
		...
		static void postCastSpell(Player* player, uint64_t manaCost, uint32_t soulCost);

		uint64_t getManaCost(const Player* player) const;
		uint64_t getSoulCost() const {
			return soul;
		}
		...
		void setMana(uint64_t m) {
			mana = m;
		}
		uint64_t getManaPercent() const {
			return manaPercent;
		}
		void setManaPercent(uint64_t m) {
			manaPercent = m;
		}
