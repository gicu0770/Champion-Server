...
enum ItemTypes_t {
	...
	ITEM_TYPE_KEY,
	ITEM_TYPE_RUNE,
	ITEM_TYPE_SUPPORTRUNE,
	ITEM_TYPE_LAST,
	ITEM_TYPE_EXTENDEDCONTIANER
};
...
enum ItemParseAttributes_t {
	...
	ITEM_PARSE_EXTENDSTORAGE,
	ITEM_PARSE_LOOTINDEX,
	...
	ITEM_PARSE_LOOTCOLOR,
	ITEM_PARSE_SPELLGEM,
	ITEM_PARSE_SPELLNAME,
	ITEM_PARSE_POTIONNAME,
	ITEM_PARSE_POTIONID,
	ITEM_PARSE_MAXENERGYSHIELD,
	ITEM_PARSE_MAXENERGYSHIELDPERCENT,
};

struct Abilities {
	uint64_t healthGain = 0;
	uint64_t healthTicks = 0;
	uint64_t manaGain = 0;
	uint64_t manaTicks = 0;
...
class ItemType
{
	public:
		...
		bool isExtendedContainer() const {
			return (type == ITEM_TYPE_EXTENDEDCONTIANER);
		}
		...
		bool isSupportRune() const {
			return (type == ITEM_TYPE_SUPPORTRUNE);
		}
		...
		uint16_t maxItems = 8;
		uint32_t slotPosition = SLOTP_HAND;
		uint16_t speed = 0;
		uint16_t wareId = 0;
		...
		uint16_t shootType = CONST_ANI_NONE;
...
class Items
{
	...
	inline const ItemType& getItemType(size_t id) const {
		if (id < items.size()) {
			return items[id];
		}
		return items.front();
	...
	inline ItemType& getItemType(size_t id) {
		if (id < items.size()) {
			return items[id];
		}
		return items.front();
	...
	inline const ItemType& getItemIdByClientId(uint16_t spriteId) const {
