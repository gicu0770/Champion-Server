...
{
	public:
		ConditionRegeneration(ConditionId_t id, ConditionType_t type, int32_t ticks, bool buff = false, uint64_t subId = 0):
			ConditionGeneric(id, type, ticks, buff, subId) {}
	...
	private:
		uint64_t internalHealthTicks = 0;
		uint64_t internalManaTicks = 0;
		uint64_t internalEnergyTicks = 0;
		
		uint64_t healthTicks = 1000;
		uint64_t manaTicks = 1000;
		uint64_t energyTicks = 1000;
		uint64_t healthGain = 0;
		uint64_t manaGain = 0;
		uint64_t energyGain = 0;
};
...
class ConditionDamage final : public Condition
{
	...
	private:
		...
		int64_t periodDamageTick = 0;
		int64_t tickInterval = 2000;
