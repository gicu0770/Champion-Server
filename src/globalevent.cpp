...
GlobalEventMap GlobalEvents::getEventMap(GlobalEvent_t type)
{
	...
	switch (type) {
		...
		case GLOBALEVENT_PREREMOVEITEM: {
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("time"))) {
		...
		} else if (strcasecmp(value, "preremoveitem") == 0) {
			eventType = GLOBALEVENT_PREREMOVEITEM;
		} else {
...
std::string GlobalEvent::getScriptEventName() const
{
	switch (eventType) {
		...
		case GLOBALEVENT_PREREMOVEITEM: return "onPreRemoveItem";
...
bool GlobalEvent::executePreRemove(Item* item, uint32_t playerId)
{
	//onPreRemoveItem(item, corpseOwner)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - GlobalEvent::executePreRemove] Call stack overflow" << std::endl;
		return false;
	}
