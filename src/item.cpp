...
Item* Item::CreateItem(const uint16_t type, uint16_t count /*= 0*/, bool loaded /*= false*/)
{
	...
	if (it.id != 0) {
		...
		} else if (it.isExtendedContainer()) {
			newItem = new ExtendStorage(type);
		...
		if (it.pickupable && !it.stackable) {
			g_game.addRealUniqueItem(g_game.nextItemUID(), newItem);
		}
...
Item* Item::clone() const
{
	...
	if (isPickupable() && !isStackable()) {
		g_game.addRealUniqueItem(g_game.nextItemUID(), item);
	}
	return item;
}
		...
		if (ItemAttributes::isStrAttrType(attribute.type)) {
			...
			for (const auto& otherAttribute : otherAttributeList) {
				if (attribute.type == otherAttribute.type && attribute.value.integer != otherAttribute.value.integer) {
					if (otherAttribute.type != ITEM_ATTRIBUTE_CUSTOM) {
						return false;
					}

					if (attribute.value.custom == nullptr || otherAttribute.value.custom == nullptr) {
						return false;
					}
					...
					{
						const auto& customMapA = *attribute.value.custom;
						const auto& customMapB = *otherAttribute.value.custom;
						auto itA = customMapA.find("rarity");
						auto itB = customMapB.find("rarity");

						if (itA == customMapA.end() || itB == customMapB.end()) {
							return false;
						}

						const auto& varA = itA->second.value;
						const auto& varB = itB->second.value;
						if (varA.type() != varB.type()) {
							return false;
						}
						if (varA.type() != typeid(int64_t)) {
							return false;
						}
						if (boost::get<int64_t>(varA) != boost::get<int64_t>(varB)) {
							return false;
						}
					}
				}
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_REALUID: {
			uint32_t realUID;
			if (!propStream.read<uint32_t>(realUID)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_REALUID, realUID);
			break;
		}
		
		case ATTR_MAXSIZE: {
			uint32_t maxSize;
			if (!propStream.read<uint32_t>(maxSize)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_MAXSIZE, maxSize);
			if (getContainer()) {
				getContainer()->setMaxSize(maxSize);
			}
			break;
		}
...
bool Item::unserializeAttr(PropStream& propStream)
{
	...
	if (getRealUID() != 0) {
		g_game.addRealUniqueItem(getRealUID(), this);
	}
	return true;
}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (it.stackable || it.isFluidContainer() || it.isSplash()) {
		propWriteStream.write<uint8_t>(ATTR_NEWCOUNT);
	...
	if (hasAttribute(ITEM_ATTRIBUTE_REALUID)) {
		propWriteStream.write<uint8_t>(ATTR_REALUID);
		propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_REALUID));
	}
	
	if (hasAttribute(ITEM_ATTRIBUTE_MAXSIZE)) {
		propWriteStream.write<uint8_t>(ATTR_MAXSIZE);
		propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_MAXSIZE));
	}
...
std::vector<std::pair<std::string, std::string>> Item::getDescriptions(const ItemType& it, const Item* item /*= nullptr*/)
{
	...
	if (item) {
		...
		} else if (it.slotPosition & SLOTP_RING2) {
			descriptions.emplace_back("Body Position", "Finger");
		} else if (it.slotPosition & SLOTP_SPELL1) {
			descriptions.emplace_back("Body Position", "Spell1");
		} else if (it.slotPosition & SLOTP_SUPPORT1_1) {
			descriptions.emplace_back("Body Position", "Support");
		} else if (it.slotPosition & SLOTP_POTION1) {
			descriptions.emplace_back("Body Position", "Potion1");
		} else if (it.slotPosition & SLOTP_POTION2) {
			descriptions.emplace_back("Body Position", "Potion2");
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			sink << "\nRange:" << (item ? item->getShootRange() : it.shootRange);
		...
		if (it.abilities) {
			...
			if (it.abilities->stats[STAT_MAXHITPOINTS]) {
                    ...
                    sink << "Max HP " <<= it.abilities->stats[STAT_MAXHITPOINTS];
                }
             ...
             if (it.abilities->statsPercent[STAT_MAXHITPOINTS]) {
                    ...
                    sink << "Max HP " <<= (it.abilities->statsPercent[STAT_MAXHITPOINTS] - 100);
					sink << '%';
                }
			if (it.abilities->stats[STAT_MAXMANAPOINTS]) {
                    ...
                    sink << "Max MP " <<= it.abilities->stats[STAT_MAXMANAPOINTS];
                }
                ...
                if (it.abilities->statsPercent[STAT_MAXMANAPOINTS]) {
                    ...
                    sink << "Max MP " <<= (it.abilities->statsPercent[STAT_MAXMANAPOINTS] - 100);
					sink << '%';
                }
                ...
                if (it.abilities->manaGain) {
                    ...
                    sink << "MP Regen " <<= it.abilities->manaGain;
                }
                ...
                if (it.abilities->healthGain) {
                    ...
                    sink << "HP Regen " <<= it.abilities->healthGain;
                }
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			sink << " mastery " << it.minReqMagicLevel << " or higher";
		}
...
void Item::setMaxSize(uint32_t n)
{
	getAttributes()->setMaxSize(n);
}
...
bool Item::isCraftItem() const
{
	static const std::unordered_set<uint16_t> craftItems = {
		8302, 8303, 37113, 37114, 37115, 37116, 37117, 37118, 37119, 37120,
		37140, 37154, 36959, 18422, 26804, 26807, 26805, 37129, 37121, 37148,
		37112, 37131, 37141, 37125, 37135, 37122, 38541, 38542, 38543, 26555,
		38496, 38409
	};
	return craftItems.find(id) != craftItems.end();
}

uint8_t Item::formatItemType() const
{
	const ItemType& itemType = items[id];
	WeaponType_t weaponType = itemType.weaponType;
	int32_t slotPosition = itemType.slotPosition - SLOTP_LEFT - SLOTP_RIGHT;
	...
	if (weaponType == WEAPON_SHIELD) {
		return 16;
	} else if (slotPosition == 1024 && (weaponType == WEAPON_SWORD || weaponType == WEAPON_CLUB || weaponType == WEAPON_AXE)) {
		return 1;
	} else if (weaponType == WEAPON_SWORD || weaponType == WEAPON_CLUB || weaponType == WEAPON_AXE) {
		return 2;
	} else if (slotPosition == 1024 && (itemType.name.find("Bow") != std::string::npos || itemType.name.find("Crossbow") != std::string::npos) && weaponType == WEAPON_DISTANCE) {
		return 3;
	} else if ((itemType.name.find("Bow") != std::string::npos || itemType.name.find("Crossbow") != std::string::npos) && weaponType == WEAPON_DISTANCE) {
		return 4;
	} else if (itemType.name.find("knife") != std::string::npos && weaponType == WEAPON_DISTANCE) {
		return 5;
	} else if (slotPosition == 1024 && weaponType == WEAPON_WAND) {
		return 6;
	} else if (weaponType == WEAPON_DISTANCE) {
		return 7;
	} else if (weaponType == WEAPON_WAND) {
		return 8;
	} else if (slotPosition == SLOTP_HEAD) {
		return 9;
		...
		return 13;
	} else if (slotPosition == SLOTP_RING || slotPosition == SLOTP_RING2) {
		return 14;
		...
		return 18;
	} else if (slotPosition == SLOTP_SUPPORT1_1) {
		return 19;
	} else if (itemType.isContainer()) {
		return 20;
	} else if (itemType.type == 9) {
		return 23;
	} else if (itemType.isUseable()) {
		return 21;
	} else if (itemType.color == 8) {
		return 24;
	}

	// Check custom attributes
	if (const ItemAttributes::CustomAttribute* attr = getCustomAttribute("crystal")) {
		return 25;
	}
