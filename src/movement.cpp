...
MoveEvent* MoveEvents::getEvent(Item* item, MoveEvent_t eventType, slots_t slot)
{
	...
	switch (slot) {
		...
		case CONST_SLOT_GLOVES: slotp = SLOTP_GLOVES; break;
		case CONST_SLOT_RING2: slotp = SLOTP_RING2; break;
		case CONST_SLOT_SPELL1: slotp = SLOTP_SPELL1; break;
		case CONST_SLOT_SPELL2: slotp = SLOTP_SPELL2; break;
		case CONST_SLOT_SPELL3: slotp = SLOTP_SPELL3; break;
		case CONST_SLOT_SPELL4: slotp = SLOTP_SPELL4; break;
		case CONST_SLOT_POTION1: slotp = SLOTP_POTION1; break;
		case CONST_SLOT_POTION2: slotp = SLOTP_POTION2; break;
	...
	if (eventType == MOVE_EVENT_EQUIP || eventType == MOVE_EVENT_DEEQUIP) {
		...
		if (slotAttribute) {
			...
			if (!tfs_strcmp(tmpStr.c_str(), "head")) {
				...
				slot = SLOTP_RING;
			} else if (!tfs_strcmp(tmpStr.c_str(), "gloves")) {
				slot = SLOTP_GLOVES;
			} else if (!tfs_strcmp(tmpStr.c_str(), "ring2")) {
				slot = SLOTP_RING2;
			} else if (!tfs_strcmp(tmpStr.c_str(), "spell1")) {
				slot = SLOTP_SPELL1;
			} else if (!tfs_strcmp(tmpStr.c_str(), "spell2")) {
				slot = SLOTP_SPELL2;
			} else if (!tfs_strcmp(tmpStr.c_str(), "spell3")) {
				slot = SLOTP_SPELL3;
			} else if (!tfs_strcmp(tmpStr.c_str(), "spell4")) {
				slot = SLOTP_SPELL4;
			} else if (!tfs_strcmp(tmpStr.c_str(), "potion1")) {
				slot = SLOTP_POTION1;
			} else if (!tfs_strcmp(tmpStr.c_str(), "potion2")) {
				slot = SLOTP_POTION2;
			} else {
...
uint32_t MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	if (!it.abilities || slot == CONST_SLOT_FORGE) {
		return 1;
	}
	...
	if (needUpdateStats || needUpdateSkills) {
		player->sendStats(1);
		player->sendStats(2);
		player->sendStats(4);
	}
