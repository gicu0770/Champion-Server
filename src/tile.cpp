		...
		if (const Monster* monster = creature->getMonster()) {
			...
			if (monster->canPushCreatures() && !monster->isSummon()) {
				...
				for (const Creature* tileCreature : *creatures) {
					...
					if (!tileCreature->isInGhostMode() && !tileCreature->isPet()) {
...
{
	MapSector* sector = g_game.map.getMapSector(tilePos.x, tilePos.y);
	if (sector) {
		sector->removeCreature(creature);
	}
...
Thing* Tile::getThing(size_t index) const
{
	...
	if (items) {
		...
		if (index < topItemSize) {
			// Bounds check: vector size can change during move events (transform/decay)
			if (index < items->size()) {
				return (*items)[index];
			}
			return nullptr;
	...
	if (items) {
		uint32_t downItemCount = items->getDownItemCount();
		if (index < downItemCount) {
			// Calculate reverse index for down items
			size_t totalSize = items->size();
			if (totalSize > 0) {
				size_t itemIndex = totalSize - (index + 1);
				// Bounds check: ensure calculated index is valid
...
void Tile::setWidget()
{
	setFlag(TILESTATE_WIDGET);
 
	const Position& cylinderMapPos = getPosition();
 
	SpectatorVector spectators;
	g_game.map.getSpectators(spectators, cylinderMapPos, false, true);
 
	//send to clients
	for (Creature* spectator : spectators) {
		spectator->getPlayer()->sendTileWidget(this, cylinderMapPos);
	}
}
 
void Tile::removeWidget()
{
	resetFlag(TILESTATE_WIDGET);
	...
	//send to clients
	for (Creature* spectator : spectators) {
		spectator->getPlayer()->sendTileRemoveWidget(this, cylinderMapPos);
	}
}