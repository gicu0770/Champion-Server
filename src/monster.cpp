...
{
	return Creature::canSee(getPosition(), pos, CLIENT_MAP_WIDTH_OFFSET - 2, CLIENT_MAP_HEIGHT_OFFFSET - 2);
}

std::string Monster::getDescription(int32_t) const
{
	std::stringExtended str(strDescription.length() + static_cast<size_t>(22));
	return (str << strDescription << ", it is level " << getLevel() << '.');
}
...
void Monster::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (spawn) {
			spawn->removeSpawnBlocker(creature);
		}
...
void Monster::updateTargetList()
{
	...
	while (friendIterator != friendList.end()) {
		...
		if (creature->isRemoved() || creature->getHealth() <= 0 || !canSee(creature->getPosition())) {
			creature->decrementReferenceCounter();
...
void Monster::onCreatureEnter(Creature* creature)
{
	...
	if (isBoss()) {
		if (Player* player = creature->getPlayer()) {
			g_events->eventPlayerOnBossAppear(player, this);
		}
	}
...
void Monster::onCreatureLeave(Creature* creature)
{
	...
	if (isBoss()) {
		if (Player* player = creature->getPlayer()) {
			g_events->eventPlayerOnBossDisappear(player, this);
		}
	}
		...
		if (canUseSpell(myPos, targetPos, spellBlock, interval, inRange, resetTicks)) {
			if (spellBlock.chance >= static_cast<uint32_t>(uniform_random(1, 100))) {
				...
				if (spellBlock.isMelee) {
					if (lastAttackedCreatureId != attackedCreature->getID()) {
						lastMeleeAttack = OTSYS_TIME() - 1000;
						lastAttackedCreatureId = attackedCreature->getID();
						return;
					}
				}
...
bool Monster::getNextStep(Direction& direction, uint32_t& flags)
{
	...
	if (hasCondition(CONDITION_STUN) || hasCondition(CONDITION_ROOT) || hasCondition(CONDITION_FEAR)) {
    return false;
	}
...
{
	if (corpse && lootDrop) {
		g_game.addRealUniqueItem(g_game.nextItemUID(), corpse);
