...
{
	if (tile->hasFlag(TILESTATE_BLOCKPROJECTILE | TILESTATE_FLOORCHANGE)) {
...
bool Combat::setParam(CombatParam_t param, const std::string& value)
{
	switch (param) {
		case COMBAT_PARAM_COLOR: {
			params.color = value;
			return true;
		}
...
bool Combat::setParam(CombatParam_t param, uint64_t value)
{
	switch (param) {
		...
		case COMBAT_PARAM_TYPEOB: {
			params.typeOb = (value != 0);
			return true;
		}
		...
		case COMBAT_PARAM_BOTTOMEFFECT: {
			params.bottomEffect = static_cast<uint8_t>(value);
			return true;
		}

		case COMBAT_PARAM_SAVEPOS: {
			params.savePos = (value != 0);
		}

		case COMBAT_PARAM_DAMAGE: {
			params.damage = static_cast<int64_t>(value);
			return true;
		}

		case COMBAT_PARAM_DAMAGE2: {
			params.damage2 = static_cast<int64_t>(value);
			return true;
		}

		case COMBAT_PARAM_TYPE2: {
			params.combatType2 = static_cast<CombatType_t>(value);
			return true;
		}

		case COMBAT_PARAM_CENTEREFFECT: {
			params.centerEffect = (value != 0);
			return true;
		}

		case COMBAT_PARAM_OFFSETXEFFECT: {
			params.offsetX = static_cast<int8_t>(value);
			return true;
		}

		case COMBAT_PARAM_OFFSETYEFFECT: {
			params.offsetY = static_cast<int8_t>(value);
			return true;
		}
	}
	return false;
...
void Combat::combatTileEffects(const SpectatorVector& spectators, Creature* caster, Tile* tile, const CombatParams& params, Position fromPos)
{
	if (params.itemId != 0) {
	...
	if (params.tileCallback) {
		params.tileCallback->onTileCombat(caster, tile, fromPos);
	}

	if (params.impactEffect != CONST_ME_NONE) {
		if (params.bottomEffect == 1) {
		...
		if (!params.centerEffect) {
			Game::addMagicEffect(spectators, tile->getPosition(), params.impactEffect, params.bottomEffect, params.color);
		}
	}
}
...
void Combat::postCombatEffects(Creature* caster, const Position& pos, const CombatParams& params)
{
	if (caster && params.distanceEffect != CONST_ANI_NONE) {
		addDistanceEffect(caster, caster->getPosition(), pos, params.distanceEffect, params.typeOb);
	}
}

void Combat::addDistanceEffect(Creature* caster, const Position& fromPos, const Position& toPos, uint16_t effect, bool type)
{
	...
	if (effect != CONST_ANI_NONE) {
		g_game.addDistanceEffect(fromPos, toPos, effect, type);
	}
}

void Combat::doCombat(Creature* caster, Creature* target, int8_t multiCast, int16_t critChance, int16_t critMulti, bool gambler, uint32_t uid) const
{
	if (multiCast >= 1) {
		g_dispatcher.addEvent(150, [this, caster, target, multiCast, critChance, critMulti, gambler, uid]() {
			doCombat(caster, target, 0, critChance, critMulti, gambler, uid);
		});
	}

	if (params.combatType != COMBAT_NONE) {
		if (!caster || caster->isRemoved() || !target || target->isRemoved()) {
			return;
		}
		CombatDamage damage = getCombatDamage(caster, target);
		if (multiCast >= 1) {
			double damage_multi[4] = {1.0, 1.0, 1.0, 1.0};
			int64_t damagePrimary = damage.primary.value * damage_multi[multiCast - 1];
			int64_t damageSecondary = damage.secondary.value * damage_multi[multiCast - 1];
			damage.primary.value = static_cast<int64_t>(damagePrimary);
			damage.secondary.value = static_cast<int64_t>(damageSecondary);
		}

		damage.critChance = critChance;
		damage.critMultiplier = critMulti;
		damage.uid = uid;
		damage.gambler = gambler;
		...
		if ((caster == target || canCombat) && params.impactEffect != CONST_ME_NONE) {
			g_game.addMagicEffect(target->getPosition(), params.impactEffect, params.bottomEffect, params.color);
		}
		...
		if (!params.aggressive || (caster != target && Combat::canDoCombat(caster, target) == RETURNVALUE_NOERROR)) {
			...
			combatTileEffects(spectators, caster, target->getTile(), params, target->getPosition());
			...
			*/
			...
			if (caster && params.distanceEffect != CONST_ANI_NONE) {
				addDistanceEffect(caster, caster->getPosition(), target->getPosition(), params.distanceEffect, params.typeOb);
			}
		}
	}
}

void Combat::doCombat(Creature* caster, const Position& position, int8_t multiCast, int16_t critChance, int16_t critMulti, bool gambler, uint32_t uid) const
{
	if (multiCast >= 1) {
		g_dispatcher.addEvent(333, [this, caster, position, multiCast, critChance, critMulti, gambler, uid]() {
			doCombat(caster, position, 0, critChance, critMulti, gambler, uid);
		});
	}
	if (params.combatType != COMBAT_NONE) {
		...
		for (Tile* tile : tileList) {
			...
			combatTileEffects(spectators, caster, tile, params, position);
				...
				if (!(casterPlayer->hasCondition(CONDITION_DISABLECRIT, 0, true))) {
					if (!damage.critical && damage.origin != ORIGIN_CONDITION && damage.origin != ORIGIN_DOT && damage.primary.type != COMBAT_HEALING) {
						...
						uint16_t chance = damage.gambler 
							? static_cast<uint16_t>(std::min(baseCrit / 2.0, 50.0))  // limit 50%
							: baseCrit;

						uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT) + damage.critMultiplier;
						...
						} else if (damage.gambler) {
							damage.primary.value = static_cast<int64_t>(std::round(damage.primary.value / 10));
							damage.secondary.value = static_cast<int64_t>(std::round(damage.secondary.value / 10));
						}
					}
				}
			}
	...
	if (success) {
		...
		if (casterPlayer && damage.origin != ORIGIN_CONDITION) {
			if (!damage.leeched) {
				...
				int64_t totalDamage = std::abs(damage.primary.value + damage.secondary.value);
				...
				if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
					...
					uint16_t limitHP = casterPlayer->getMaxHealth() * 0.05;
					if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
						leechCombat.primary.value = std::round(totalDamage * (skill / 100.));
						if (leechCombat.primary.value > limitHP) {
							leechCombat.primary.value = limitHP;
						}
				...
				int64_t mana = static_cast<int64_t>(casterPlayer->getMana());
				if (mana < casterPlayer->getMaxMana()) {
					...
					uint16_t limitHP = casterPlayer->getMaxMana() * 0.05;
					if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
						leechCombat.primary.value = std::round(totalDamage * (skill / 100.));
						if (leechCombat.primary.value > limitHP) {
							leechCombat.primary.value = limitHP;
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	if (params.centerEffect) {
		const Position& offsetPosition = Position(position.x + params.offsetX, position.y + params.offsetY, position.z);
		g_game.addMagicEffect(offsetPosition, params.impactEffect, params.bottomEffect, params.color);
	}
	...
	for (Creature* creature : toDamageCreatures) {
		// Validate creature is still valid (may have been removed by previous iteration or Lua callback)
		if (!creature || creature->isRemoved()) {
			continue;
		}
		...
		if (casterPlayer && !damage.critical && damage.origin != ORIGIN_CONDITION && damage.primary.type != COMBAT_HEALING) {
			if (!(casterPlayer->hasCondition(CONDITION_DISABLECRIT, 0, true))) {
				...
				uint16_t chance = damage.gambler
					? static_cast<uint16_t>(std::min(std::round((casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE) + damage.critChance) / 2.0), 50.0))
					: static_cast<uint16_t>(std::round(casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE) + damage.critChance));
				...
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
					...
					damageCopy.primary.value = static_cast<int64_t>(std::round(damageCopy.primary.value / 10));
					damageCopy.secondary.value = static_cast<int64_t>(std::round(damageCopy.secondary.value / 10));
				}
			}
		}
		...
		if (success) {
			...
			int64_t totalDamage = std::abs(damageCopy.primary.value + damageCopy.secondary.value);
			...
			if (casterPlayer && !damage.leeched && damage.origin != ORIGIN_CONDITION) {
				...
				if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
					uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHCHANCE);
					uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT);
					uint16_t limitHP = casterPlayer->getMaxHealth() * 0.03;
					if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
				...
				if (mana < casterPlayer->getMaxMana()) {
					uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE);
					uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT);
					uint16_t limitHP = casterPlayer->getMaxMana() * 0.03;
					if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
						leechCombat.primary.value = std::ceil(totalDamage * ((skill / 100.) + ((targetsCount - 1) * ((skill / 100.) / 10.))) / targetsCount);
						if (leechCombat.primary.value > limitHP) {
							leechCombat.primary.value = limitHP;
...
void TileCallback::onTileCombat(Creature* creature, Tile* tile, Position fromPos) const
{
