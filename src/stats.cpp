...
static std::string extractFilenameFromStackTrace(const std::string& stackTrace)
{
	// Stack trace format example:
	// "	[C]: in function 'callback'\n\tdata/scripts/spells/spells/tornado.lua:85: in function <data/scripts/spells/spells/tornado.lua:70>"
	
	// Look for .lua file in the stack trace
	size_t luaPos = stackTrace.find(".lua");
	if (luaPos == std::string::npos) {
		return "";
	}
	
	// Find start of the path (look backwards for space, newline, or start of string)
	size_t start = stackTrace.rfind('\n', luaPos);
	if (start == std::string::npos) {
		start = stackTrace.rfind('\t', luaPos);
	}
	if (start == std::string::npos) {
		start = stackTrace.rfind(' ', luaPos);
	}
	...
	// Find end (colon after .lua or end of extension)
	size_t end = luaPos + 4; // length of ".lua"
	
	std::string filepath = stackTrace.substr(start, end - start);
...
void Stats::writeStats(const std::string& file, const statsMap& stats, const std::string& extraInfo) const
{
	...
	for (auto& it : pairs) {
		...
		if (percent > 0.1) {
			...
			// For LuaAddEvent entries, try to extract filename from stack trace
			if (description.find("LuaAddEvent") == 0 && !it.second.extraInfo.empty()) {
				std::string filename = extractFilenameFromStackTrace(it.second.extraInfo);
				if (!filename.empty()) {
			...
			out << std::setw(10) << it.second.executionTime / 1000000 << std::setw(10) << it.second.calls
			    ...
			    << std::setprecision(5) << std::fixed << realPercent << "%" << " " << description << "\n";
		}
	}
	out << "\n";
	out.flush();
