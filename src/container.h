...
class Container : public Item, public Cylinder
{
	public:
		...
		virtual TempStorage* getTempStorage() {
			return nullptr;
		}
		virtual const TempStorage* getTempStorage() const {
			return nullptr;
		}

		virtual TradeStorage* getTradeStorage() {
			return nullptr;
		}
		virtual const TradeStorage* getTradeStorage() const {
			return nullptr;
		}
		...
		uint32_t setMaxSize(uint32_t size) {
			return maxSize = size;
		}
		...
		Item* getItemById(uint32_t id) const;
		Item* getItemByMarketId(int64_t UID) const;
		...
		void decrementSerializationCount() {
			if (serializationCount > 0) {
				--serializationCount;
			}
		}

		void sortByRarity(bool ascending = true, const std::string& nameToFind = "");
		void sortByCount(bool ascending = true, const std::string& nameToFind = "");

	protected:
		ItemDeque itemlist;

	private:
		std::string& getContentDescription(std::stringExtended& sink, bool colored = false) const;
		...
		void onUpdateContainerItem(uint32_t index, Item* item);
		void onRemoveContainerItem(uint32_t index);
