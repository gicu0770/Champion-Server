...
void DatabaseTasks::addTask(std::string query, std::function<void(DBResult_ptr, bool, uint64_t)> callback/* = nullptr*/, bool store/* = false*/)
{
	bool signal = false;
	taskLock.lock();
...
void DatabaseTasks::runTask(const DatabaseTask& task)
{
	...
	if (task.callback) {
		g_dispatcher.addTask(std::bind(task.callback, result, success, db.getAffectedRows()));
	}
}
