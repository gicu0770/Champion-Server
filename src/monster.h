...
class Monster final : public Creature
{
	public:
		...
		bool isSpawnBlocking(Creature* creature) const override {
			if (spawn) {
				return spawn->isSpawnBlocking(creature);
			}
			return false;
		}
		...
		int32_t getTier() const {
			return mType->info.tier;
		}
		const std::string& getItems() const {
			return mType->info.items;
		}
		...
		uint32_t getZoneId() const override {
			return mType->info.zoneId;
		}

		uint32_t getBestiaryId() const override {
			return mType->info.bestiaryId;
		}
	...
	private:
		...
		uint64_t lastAttackedCreatureId = 0;
