...
void connectToMainServer()
{
    ...
    auto server = std::make_shared<MainServerConnection>(io, "51.222.245.108", 6666);
    server->start();

	std::thread([io_ptr = &io]() { io_ptr->run(); }).detach();
}
...
void mainLoader(int argc, char* argv[], ServiceManager* services)
{
	...
	if (type != 0) {
		...
		std::cout << ">> Started instance with port: " << port << "\n>> Forced Map: " << mapName << "\n>> Server type " << type << std::endl;
		g_config.setString(ConfigManager::MAP_NAME, mapName);
		g_config.setNumber(ConfigManager::GAME_PORT, stoi(port));
		g_config.setNumber(ConfigManager::INSTANCE_TYPE, type);
	}
	...
	std::cout << ">> Loading lastRealUID" << std::endl;
	g_game.setGameState(GAME_STATE_PREINIT);
	...
	Cams::CAMS_DIR = g_config.getString(ConfigManager::CAMS_DIR);
	std::clog << ">> Cams dir is set to: " << Cams::CAMS_DIR << std::endl;
	...
	std::cout << ">> Server Instance Type: " << g_config.getNumber(ConfigManager::INSTANCE_TYPE) << std::endl;
