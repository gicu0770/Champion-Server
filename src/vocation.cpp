...
bool Vocations::loadFromXml()
{
	...
	for (auto vocationNode : doc.child("vocations").children()) {
		...
		if ((attr = vocationNode.attribute("gainhp"))) {
			voc.gainHP = pugi::cast<uint64_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainmana"))) {
			voc.gainMana = pugi::cast<uint64_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainhpticks"))) {
			voc.gainHealthTicks = pugi::cast<uint64_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainhpamount"))) {
			voc.gainHealthAmount = pugi::cast<uint64_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainmanaticks"))) {
			voc.gainManaTicks = pugi::cast<uint64_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainmanaamount"))) {
			voc.gainManaAmount = pugi::cast<uint64_t>(attr.value());
		}
...
uint32_t Vocation::skillBase[SKILL_LAST + 1] = {50, 50, 50, 50, 50, 50, 50};
