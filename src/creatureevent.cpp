...
void CreatureEvents::clear(bool fromLua)
{
	...
	for (auto it = reconnectEvents.begin(); it != reconnectEvents.end(); ) {
		if (fromLua == (*it).fromLua) {
			it = reconnectEvents.erase(it);
		} else {
			++it;
		}
	}
...
bool CreatureEvents::registerEvent(Event_ptr event, const pugi::xml_node&)
{
	...
	if (creatureEvent->getEventType() == CREATURE_EVENT_LOGIN) {
		...
		return true;
	} else if (creatureEvent->getEventType() == CREATURE_EVENT_RECONNECT) {
		reconnectEvents.emplace_back(std::move(*creatureEvent));
		return true;
	} else {
...
bool CreatureEvents::playerLogin(Player* player) const
{
	...
	for (const auto& it : loginEvents) {
		if (!it.executeOnLogin(player)) {
			std::cout << "[Debug - CreatureEvents::playerLogin] Login event '" << it.getName() << "' returned false for player " << player->getName() << std::endl;
			return false;
		}
	}
	return true;
...
bool CreatureEvents::playerReconnect(Player* player) const
{
	//fire global event if is registered
	for (const auto& it : reconnectEvents) {
		if (!it.executeOnReconnect(player)) {
			return false;
		}
	}
	return true;
...
bool CreatureEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (!tfs_strcmp(tmpStr.c_str(), "login")) {
		...
		type = CREATURE_EVENT_ENDSCRIPTS_REGEN;
	} else if (!tfs_strcmp(tmpStr.c_str(), "minusregenhp")) {
		type = CREATURE_EVENT_MINUSREGENHP;
	} else if (!tfs_strcmp(tmpStr.c_str(), "extendedopcode")) {
		type = CREATURE_EVENT_EXTENDED_OPCODE;
	} else if (!tfs_strcmp(tmpStr.c_str(), "onhealthdraing")) {
		type = CREATURE_EVENT_HEALTHDRAIN;
	} else {
...
std::string CreatureEvent::getScriptEventName() const
{
	...
	switch (type) {
		...
		case CREATURE_EVENT_ENDSCRIPTS_REGEN:
			return "onEndScriptsRegen";

		case CREATURE_EVENT_MINUSREGENHP:
			return "onMinusRegenHp";

		case CREATURE_EVENT_EXTENDED_OPCODE:
			return "onExtendedOpcode";

		case CREATURE_EVENT_HEALTHDRAIN:
			return "onHealthDrain";
...
bool CreatureEvent::executeOnReconnect(Player* player) const
{
	//onRecoonect(player)
	if (!scriptInterface->reserveScriptEnv()) {
...
{
	//onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin, critical, uid, critChance, distance)
	if (!creature || creature->isRemoved()) {
		std::cout << "[Error - CreatureEvent::executeHealthChange] Creature is null or removed" << std::endl;
		return;
	}
	...
	if (attacker) {
		...
		distance = std::max(std::abs(creaturePos.x - attackerPos.x), std::abs(creaturePos.y - attackerPos.y));
	}
	lua_pushnumber(L, distance);

	if (scriptInterface->protectedCall(L, 11, 4) != 0) {
...
void CreatureEvent::executeEndScriptsRegen(Player* player, bool health)
{
	//onEndScriptsRegen(player, health)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeEndScriptsRegen] Call stack overflow" << std::endl;
		return;
	}
	...
	LuaScriptInterface::setMetatable(L, -1, "Player");
	LuaScriptInterface::pushBoolean(L, health);

	scriptInterface->callVoidFunction(2);
}

void CreatureEvent::executeMinusRegenHP(Player* player, int32_t value)
{
	//onMinusRegenHp(player, value)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeMinusRegenHp] Call stack overflow" << std::endl;
		return;
	}
...
void CreatureEvent::executeOnHealthDrain(Creature* creature, Creature* attacker, int64_t damage)
{
	if (!creature || creature->isRemoved()) {
		std::cout << "[Error - CreatureEvent::executeOnHealthDrain] Creature is null or removed" << std::endl;
		return;
	}

	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeOnHealthDrain] Call stack overflow" << std::endl;
		return;
	}
...
{
	//onManaChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin, critical)
	if (!creature || creature->isRemoved()) {
		std::cout << "[Error - CreatureEvent::executeManaChange] Creature is null or removed" << std::endl;
		return;
	}
	...
	if (scriptInterface->protectedCall(L, 10, 4) != 0) {
		LuaScriptInterface::reportError(nullptr, LuaScriptInterface::popString(L));
	} else {
