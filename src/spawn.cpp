...
bool Spawns::loadFromXml(const std::string& filename, int8_t instance)
{
	if (loaded) {
		return true;
	}
	...
	for (auto spawnNode : doc.child("spawns").children()) {
		Position centerPos(
			...
			pugi::cast<uint16_t>(spawnNode.attribute("centerz").value()) + (16 * instance)
		);
...
bool Spawn::spawnMonster(spawnBlock_t& sb, bool startup /*= false*/)
{
	...
	if (!g_events->eventMonsterOnSpawn(monster_ptr.get(), sb.pos, startup, false)) {
		return false;
	}
...
void Spawn::addSpawnBlocker(Creature* creature)
{
	auto result = spawnBlockers.insert(creature);
	if (result.second) {
		creature->incrementReferenceCounter();
	}
}

void Spawn::removeSpawnBlocker(Creature* creature)
{
	auto it = spawnBlockers.find(creature);
	if (it != spawnBlockers.end()) {
		creature->decrementReferenceCounter();
		spawnBlockers.erase(it);
	}

	it = preSpawnBlockers.find(creature);
	if (it != preSpawnBlockers.end()) {
		creature->decrementReferenceCounter();
		preSpawnBlockers.erase(it);
	}
}
...
void Spawn::addPreSpawnBlocker(Creature* creature, Monster* monster)
{
	if (isPreSpawnBlocking(creature)) {
		...
		if (Player* player = creature->getPlayer()) {
			player->sendCreatureAttackable(monster, false);
		}
		return;
	}

	auto result = preSpawnBlockers.insert(creature);
	if (result.second) {
		creature->incrementReferenceCounter();
	}
}
...
void Spawn::checkSpawnBlockers(Monster* monster)
{
	...
	for (Creature* spectator : spectators) {
		if (monster->canSee(spectator->getPosition())) {
			if (spectator->getPosition().z == centerPos.z) {
				if (Player* player = spectator->getPlayer()) {
					addPreSpawnBlocker(spectator, monster);
				}
			}
		}
	}
	...
	for (Creature* creature : spawnBlockers) {
		if (creature && !monster->canSee(creature->getPosition())) {
			toRemove.push_back(creature);
		}
	}
	
	for (Creature* creature : toRemove) {
		removeSpawnBlocker(creature);
	}
	
}

void Spawn::clearSpawnBlockers()
{
	for (Creature* creature : spawnBlockers) {
		creature->decrementReferenceCounter();
	}

	for (Creature* creature : preSpawnBlockers) {
		creature->decrementReferenceCounter();
	}

	spawnBlockers.clear();
	preSpawnBlockers.clear();
}