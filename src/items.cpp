...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"extendstorage", ITEM_PARSE_EXTENDSTORAGE},
	{"lootindex", ITEM_PARSE_LOOTINDEX},
	...
	{"maxenergyshield", ITEM_PARSE_MAXENERGYSHIELD},
	{"maxenergyshieldpercent", ITEM_PARSE_MAXENERGYSHIELDPERCENT},
	...
	{"colored", ITEM_PARSE_LOOTCOLOR},
	{"spellgem", ITEM_PARSE_SPELLGEM},
	{"spell", ITEM_PARSE_SPELLNAME},
	{"potion", ITEM_PARSE_POTIONNAME},
	{"potionid", ITEM_PARSE_POTIONID},
	{"type", ITEM_PARSE_TYPE},
	{"description", ITEM_PARSE_DESCRIPTION},
	{"runespellname", ITEM_PARSE_RUNESPELLNAME},
	{"weight", ITEM_PARSE_WEIGHT},
...
bool Items::loadFromOtbLegacy(OTB::Loader& loader, const OTB::Node& rootNode)
{
	...
	auto translateOTBSlot = [](slotsOTB_t st) {
		switch(st)
		{
			...
			case OTB_SLOT_GLOVES: return SLOTP_GLOVES;
			case OTB_SLOT_RING2: return SLOTP_RING2;
			case OTB_SLOT_SPELL1: return SLOTP_SPELL1;
			case OTB_SLOT_SPELL2: return SLOTP_SPELL2;
			case OTB_SLOT_SPELL3: return SLOTP_SPELL3;
			case OTB_SLOT_SPELL4: return SLOTP_SPELL4;
			case OTB_SLOT_POTION1: return SLOTP_POTION1;
			case OTB_SLOT_POTION2: return SLOTP_POTION2;
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				case ITEM_PARSE_TYPE: {
					...
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "support")) {
						it.type = ITEM_TYPE_SUPPORTRUNE;
					} else {
				...
				case ITEM_PARSE_SPELLNAME: {
					it.spellName = valueAttribute.as_string();
					break;
				}

				case ITEM_PARSE_POTIONNAME: {
					it.potionName = valueAttribute.as_string();
					break;
				}
				...
				case ITEM_PARSE_COLOR: {
					it.color = pugi::cast<uint32_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_CORPSETYPE: {
					...
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "boss")) {
						it.corpseType = RACE_BOSS;
					} else {
				...
				case ITEM_PARSE_EXTENDSTORAGE: {
					it.type = ITEM_TYPE_EXTENDEDCONTIANER;
					break;
				}

				case ITEM_PARSE_LOOTINDEX: {
					it.lootIndex = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_SLOTTYPE: {
					...
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "gloves")) {
						it.slotPosition |= SLOTP_GLOVES;
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "ring2")) {
						it.slotPosition |= SLOTP_RING2;
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "spell1")) {
						it.slotPosition |= SLOTP_SPELL1;
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "support")) {
						it.slotPosition |= SLOTP_SUPPORT1_1;
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "potion1")) {
						it.slotPosition |= SLOTP_POTION1;
					} else if (!tfs_strcmp(tmpStrValue.c_str(), "potion2")) {
						it.slotPosition |= SLOTP_POTION2;
					} else {
				...
				case ITEM_PARSE_SHOOTTYPE: {
					...
					if (shoot != CONST_ANI_NONE) {
						...
						it.shootType = pugi::cast<uint16_t>(valueAttribute.value());
					}
					break;
				}
				...
				case ITEM_PARSE_EFFECT: {
					...
					if (effect != CONST_ME_NONE) {
						...
						it.magicEffect = pugi::cast<uint16_t>(valueAttribute.value());
					}
					break;
				}
				...
				// 	abilities.healthGain = pugi::cast<uint64_t>(valueAttribute.value());
				// 	break;
				// }

				// case ITEM_PARSE_HEALTHTICKS: {
				// 	abilities.regeneration = true;
				// 	abilities.healthTicks = pugi::cast<uint64_t>(valueAttribute.value());
				// 	break;
				// }

				// case ITEM_PARSE_MANAGAIN: {
				// 	abilities.regeneration = true;
				// 	abilities.manaGain = pugi::cast<uint64_t>(valueAttribute.value());
				// 	break;
				// }

				// case ITEM_PARSE_MANATICKS: {
				// 	abilities.regeneration = true;
				// 	abilities.manaTicks = pugi::cast<uint64_t>(valueAttribute.value());
				// 	break;
				// }
				...
				// case ITEM_PARSE_MAXENERGYSHIELD: {
				// 	abilities.stats[STAT_MAXENERGYSHIELD] = pugi::cast<int32_t>(valueAttribute.value());
				// 	break;
				// }

				// case ITEM_PARSE_MAXENERGYSHIELDPERCENT: {
				// 	abilities.statsPercent[STAT_MAXENERGYSHIELD] = pugi::cast<int32_t>(valueAttribute.value());
				// 	break;
				// }
					...
					if (combatType != COMBAT_NONE) {
						...
						for (auto subAttributeNode : attributeNode.children()) {
							...
							if (!tfs_strcmp(tmpStrValue.c_str(), "ticks")) {
								...
								int64_t damage = -pugi::cast<int64_t>(subValueAttribute.value());
								if (start > 0) {
				...
				case ITEM_PARSE_LOOTCOLOR: {
					it.isColored = booleanString(valueAttribute.as_string());
					break;
				}

				case ITEM_PARSE_SPELLGEM: {
					it.spellGem = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_POTIONID: {
					it.potionId = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}
