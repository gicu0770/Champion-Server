...
struct spellBlock_t {
	...
	spellBlock_t(spellBlock_t&& rhs) noexcept : spell(rhs.spell), chance(rhs.chance), speed(rhs.speed), range(rhs.range), minCombatValue(rhs.minCombatValue),
		maxCombatValue(rhs.maxCombatValue), nameSpell(rhs.nameSpell), radius(rhs.radius), length(rhs.length), spread(rhs.spread), target(rhs.target),
		combatSpell(rhs.combatSpell), isMelee(rhs.isMelee) {rhs.spell = nullptr;}
...
class MonsterType
{
	struct MonsterInfo {
		...
		std::vector<uint32_t> playersBlock;
...
class Monsters
{
	public:
		...
		MonsterType* getMonsterTypeByBestiaryId(uint32_t id);
		std::map<std::string, MonsterType*>* getMonstersTypeByZoneId(uint32_t id);
	...
	private:
		...
		std::map<uint32_t, std::map<std::string, MonsterType*>> zoneMonsters;
		std::map<uint32_t, MonsterType*> bestiaryMonsters;
