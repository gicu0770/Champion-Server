...
class NpcEventsHandler
{
	public:
		...
		void onPlayerTrade(Player* player, int32_t callback, uint16_t itemId, uint8_t count, uint8_t amount, uint64_t realUID, bool ignore = false, bool inBackpacks = false);

		void onPlayerSellMultiple(Player* player, int32_t callback, const std::vector<uint64_t> items);
	...
	private:
		...
		int32_t playerSellMultipleEvent = -1;
...
class Npc final : public Creature
{
	public:
		...
		bool isSpawnBlocking(Creature* creature) const override {
			return false;
		}
