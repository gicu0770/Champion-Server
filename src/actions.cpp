...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, bool isHotkey)
{
	...
	if (Container* container = item->getContainer()) {
		...
		uint32_t itemCorpse = item->getID();
		if (itemCorpse == 27195) {
			g_game.transformItem(item, 27193);
			if (corpseOwner != 0) {
				item->setCorpseOwner(corpseOwner);
			}
		} else if (itemCorpse == 27196) {
			return RETURNVALUE_CANNOTUSETHISOBJECT;
		} else if (itemCorpse == 36731 || itemCorpse == 37014 || itemCorpse == 37015 || itemCorpse == 37016 || itemCorpse == 37017 || itemCorpse == 37018 || itemCorpse == 37019 || itemCorpse == 37013) {
			g_game.transformItem(item, 27193);
			if (corpseOwner != 0) {
				item->setCorpseOwner(corpseOwner);
			}
