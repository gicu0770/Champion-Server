local config = {
    ...
    ['loot'] = {skillKey = 17588, timeKey = 17592}
}
...
function onSay(player, words, param)
    ...
    if split[3] == nil then
        player:sendCancelMessage("Insufficient parameters [(exp,skill,magic,loot),percentage,hours].")
        return false
    end
    ...
    if not globalStorage then
        player:sendCancelMessage("Skill name value must be one of the following: exp, skill, magic, loot.")
        return false
    end
