...
function onSay(player, words, param)
    ...
    if split[1] == nil then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "[BankSystem]: The commands are:\n!bank balance\n!bank transfer, gold_amount, player_name")
        return
    end
    --------------------------- Balance ---------------------------
    if split[1] == 'balance' then
	local bal = comma_value(player:getBankBalance())
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "[BankSystem]: Your account balance is " .. bal .. "")
    --------------------------- Deposit ---------------------------
