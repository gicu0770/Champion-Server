...
if Modules == nil then
	...
	function ShopModule:addBuyableItem(names, itemid, cost, itemSubType, realName, func, level)
		if not BUYABLE_ITEMS_BY_ID[itemid] then
			BUYABLE_ITEMS_BY_ID[itemid] = {}
		end

		table.insert(BUYABLE_ITEMS_BY_ID[itemid], Npc():getName())
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
			...
			if shopItem == nil then
				if level then
					if not self.npcHandler.shopItemsLevelBased[level] then
						self.npcHandler.shopItemsLevelBased[level] = {}
					end
					table.insert(self.npcHandler.shopItemsLevelBased[level], {id = itemid, buy = cost, sell = -1, subType = itemSubType, name = realName or ItemType(itemid):getName(), func = func})
				else
					self.npcHandler.shopItems[#self.npcHandler.shopItems + 1] = {id = itemid, buy = cost, sell = -1, subType = itemSubType, name = realName or ItemType(itemid):getName(), func = func}
				end
			else
				shopItem.buy = cost
			end
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		local a, b = doNpcSellItem(cid, itemid, amount, subType, ignoreCap, inBackpacks, backpack, shopItem.func)
		if a < amount then
			...
			if a > 0 then
				removeMoneyNpc(cid, ((a * shopItem.buy) + (b * 20)))
				return true
			end
	...
	function ShopModule:callbackOnSellMultiple(cid, items)
		local player = Player(cid)
		if not player then
			print("[ShopModule.onSellMultiple] player == nil")
			return false
		end
		...
		for i = 1, #items do
			...
			if item then
				...
				if player ~= parent then
					local succes, earn = item:sellItem(player, self, cid, items[i], false)
					if succes then
						money = money + earn
						count = count + 1
					end
		...
		local msg = "Sold items x".. count .. " for " .. money .. " gold."
		doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, msg)
		return true
	end
	...
	function Item:sellItem(player, shopModule, cid, realUID, sendMesssage)
		...
		if money > 0 and player:removeItemUnique(realUID) then
			...
			if sendMesssage then
				local msg = "Sold " .. 1 .. "x " .. name .. " for " .. money .. " gold."
				doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, msg)
			end
			player:refreshBalance()
			shopModule.npcHandler.talkStart[cid] = os.time()
			return true, money
		else
			shopModule.npcHandler.talkStart[cid] = os.time()
			return false, 0
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, realUID, ignoreEquipped)
		...
		local player = Player(cid)
		if realUID > 0 then
			local item = player:getItem(realUID)
			if not item then
				return false
			end
			...
			item:sellItem(player, self, cid, realUID, true)
		end
	end
	...
	function ShopModule.requestTrade(cid, message, keywords, parameters, node)
		...
		if player then
			...
			for level, items in pairs(module.npcHandler.shopItemsLevelBased) do
				if playerLevel >= level then
					for i = 1, #items do
						itemWindow[#itemWindow + 1] = items[i]
					end
				end
			end
		end
		...
		openShopWindow(cid, itemWindow,
			...
			function(cid, itemid, subType, amount, realUID, ignoreCap, inBackpacks) module.npcHandler:onSell(cid, itemid, subType, amount, realUID, ignoreCap, inBackpacks) end)
		module.npcHandler:say(msg, cid)
		return true
	end
	...
	function ShopModule.onConfirm(cid, message, keywords, parameters, node)
		...
		elseif shop_eventtype[cid] == SHOPMODULE_BUY_ITEM then
			...
			if a < shop_amount[cid] then
				...
				module.npcHandler:say(msg, cid)
				if a > 0 then
					removeMoneyNpc(cid, a * shop_cost[cid])
