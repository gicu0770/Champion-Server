...
local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[97].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[97].manaCost,
  spellId = 97,
  range = GLOBAL_SPELL_COOLDOWNS[97].range,
  aggressive = true,
  forwardCast = true,
  cooldown = GLOBAL_SPELL_COOLDOWNS[97].cooldown,
  type = COMBAT_ICEDAMAGE,
  targets = GLOBAL_SPELL_COOLDOWNS[97].split,
...
local function onCastSpell(player, item, getInfoOnly, force, mousePos)
  ...
  local extraFunc = function(player, target)
    if math.random(100) <= 20 then
      ...
      conditionHaste:setParameter(CONDITION_PARAM_SUBID, 777788)
