...
local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[86].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[86].manaCost,
  spellId = 86,
  range = GLOBAL_SPELL_COOLDOWNS[86].range,
  aggressive = true,
  forwardCast = true,
  cooldown = GLOBAL_SPELL_COOLDOWNS[86].cooldown,
  type = COMBAT_PHYSICALDAMAGE,
  targets = GLOBAL_SPELL_COOLDOWNS[86].split,
...
local function onCastSpell(player, item, getInfoOnly, force, mousePos)
  ...
  local extraFunc = function(player, target)
    target:addBuff(COUNTER_WEAKNESS_SPELL)
  end
