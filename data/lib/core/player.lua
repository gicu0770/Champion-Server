...
function Player.isPremium(self)
	return configManager.getBoolean(configKeys.FREE_PREMIUM)
end
...
function Player.sendTooltipMessage(self, message, errorMsg)
	if errorMsg then
		return self:sendTextMessage(MESSAGE_TUTORIAL, message)
	end
	return self:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, message)
end
...
function Player.sendExtendedOpcode(self, opcode, buffer)
	if not self or self == nil or self:isRemoved() or not opcode or not buffer then
		return false
	end
...
function Player.sendAdventurerBlessing(self)
		if not self or self == nil or self:isRemoved() then
			return false
		end
...
function Creature.setExhaustion(self, value, time)
    self:setStorageValue(value, time + os.time())
end

function Creature.getExhaustion(self, value)
...
function Creature:hasExhaustion(value)
