function Container.isContainer(self)
	return true
end

function Container.createLootItem(self, item)
	if self:getEmptySlots() == 0 then
		return true
	end

	local itemCount = nil
	local randvalue = getLootRandom()
	if randvalue < item.chance then
		local it = ItemType(item.itemId)
		if it:isStackable() then
			itemCount = math.min(randvalue % item.maxCount + 1, 100)
		elseif it:isFluidContainer() then
			itemCount = item.maxCount
		else
			itemCount = 1
		end
	end

	if itemCount then
		local tmpItem = Game.createItem(item.itemId, itemCount)
		if not tmpItem then
			return false
		end

		if Game.isGlobalBuffActive(BUFF_GLOBAL_GOLD) then
			local gold = 0
			local plat = 0
			local cc = 0

			if item.itemId == ITEM_GOLD_COIN then
				gold = gold + itemCount
			elseif item.itemId == ITEM_PLATINUM_COIN then
				gold = gold + itemCount * 100
			elseif item.itemId == ITEM_CRYSTAL_COIN then
				gold = gold + itemCount * 10000
			end

			if gold > 0 then
				gold = math.floor(gold * 0.5)

				while gold >= 10000 do
					gold = gold / 10000
					cc = cc + 1
				end

				while cc > 0 do
					local n = math.min(cc, 100)
					local ccItem = Game.createItem(ITEM_CRYSTAL_COIN, n)
					cc = cc - n

					if ccItem then
						self:addItemEx(ccItem)
					end
				end

				while gold >= 100 do
					gold = gold / 100
					plat = plat + 1
				end

				while plat > 0 do
					local n = math.min(plat, 100)
					local platItem = Game.createItem(ITEM_PLATINUM_COIN, n)
					plat = plat - n

					if platItem then
						self:addItemEx(platItem)
					end
				end

				while gold > 0 do
					local n = math.min(gold, 100)
					local goldItem = Game.createItem(ITEM_GOLD_COIN, n)
					gold = gold - n

					if goldItem then
						self:addItemEx(goldItem)
					end
				end
			end
		end

		if tmpItem:isContainer() then
			local childLoot = #item.childLoot
			for i = 1, childLoot do
				if not tmpItem:createLootItem(item.childLoot[i]) then
					return false
				end
			end

			if tmpItem:getSize() == 0 and childLoot > 0 then
				return true
			end
		end

		if item.subType ~= -1 then
			tmpItem:setAttribute(ITEM_ATTRIBUTE_CHARGES, item.subType)
		end

		if item.actionId ~= -1 then
			tmpItem:setActionId(item.actionId)
		end

		if item.text and item.text ~= "" then
			tmpItem:setText(item.text)
		end

		self:addItemEx(tmpItem)
	end
	return true
end