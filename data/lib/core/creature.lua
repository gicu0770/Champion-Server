...
function Creature.getClosestFreePositionPZ(self, position, maxRadius, mustBeReachable)
	maxRadius = maxRadius or 1
	...
	for radius = 0, maxRadius do
		...
		for i = 1, total do
			...
			local tile = Tile(checkPosition)
			if tile and tile:getCreatureCount() == 0 and not tile:hasProperty(CONST_PROP_IMMOVABLEBLOCKSOLID) and not tile:hasFlag(TILESTATE_PROTECTIONZONE) and
				(not mustBeReachable or self:getPathTo(checkPosition)) then
				return checkPosition
			end
		end
