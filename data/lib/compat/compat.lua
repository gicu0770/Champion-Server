...
do
	local function CreatureEventNewIndex(self, key, value)
		...
		elseif key == "onExtendedOpcode" then
			...
			return
		elseif key == "onEndScriptsRegen" then
			self:type("endscriptsregen")
			self:onEndScriptsRegen(value)
			return
		elseif key == "onMinusRegenHP" then
			self:type("minusregenhp")
			self:onMinusRegenHP(value)
			return
		elseif key == "onHealthDrain" then
			self:type("onhealthdrain")
			self:onHealthDrain(value)
			return
...
function getPlayerWings(cid, wingId) local p = Player(cid) return p and p:hasWings(wingId) or false end
function getPlayerAura(cid, auraId) local p = Player(cid) return p and p:hasAura(auraId) or false end
...
function doPlayerAddWings(cid, wingId) local p = Player(cid) return p and p:addWings(wingId) or false end
function doPlayerAddAura(cid, auraId) local p = Player(cid) return p and p:addAura(auraId) or false end
...
function doCreateItemEx(itemid, count, extraFunc, player)
	local item = Game.createItem(itemid, count)
	if item then
		if extraFunc then
			extraFunc(item, player)
		end
