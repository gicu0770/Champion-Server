...
function door.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif table.contains(levelDoors, itemId) then
		...
		if item.actionid > 0 and player:getLevel() >= item.actionid - 1000 then
			...
			local lvlDoorrr = item.actionid - 1000
			player:sendTextMessage(MESSAGE_INFO_DESCR, "Only the worthy may pass. Level "..lvlDoorrr.." or higher.")
		end
		--]]
		item:transform(itemId + 1)
		player:teleportTo(toPosition, true)
		local lvlDoorrr = item.actionid - 1000
		player:sendTextMessage(MESSAGE_INFO_DESCR, "The recommended level for this area is: "..lvlDoorrr.." or higher.")
		return true
