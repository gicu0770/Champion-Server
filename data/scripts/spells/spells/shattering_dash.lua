local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[62].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[62].manaCost,
  spellId = 62,
  range = GLOBAL_SPELL_COOLDOWNS[62].range,
  aggressive = true,
  targets = 3,
  needTarget = false,
  cooldown = GLOBAL_SPELL_COOLDOWNS[62].cooldown,
...
local function executeFlickerStrike(player, target, combat, CONFIG_SUP, item, free, effectUnique)
  ...
  if effectUnique == 720 then
    Position(target:getPosition().x + 2, target:getPosition().y + 2, target:getPosition().z):sendMagicEffect(effectUnique, 1)
  else
...
local function startFlickerStrike(player, combat, CONFIG, CONFIG_SUP, item, effectUnique)
  ...
  local checkPathing = player:getPathTo(targetPos, 0, 1, false, false, nil, true)
