...
local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[102].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[102].manaCost,
  spellId = 102,
  range = GLOBAL_SPELL_COOLDOWNS[102].range,
  aggressive = true,
  forwardCast = true,
  cooldown = GLOBAL_SPELL_COOLDOWNS[102].cooldown,
  type = COMBAT_EARTHDAMAGE,
  targets = GLOBAL_SPELL_COOLDOWNS[102].split,
  dmgInfo = "over 2.5s",
...
local function onCastSpell(player, item, getInfoOnly, force, mousePos)
  ...
  local extraFunc = function(player, target)
    target:startDOT(player, TOXIC_SPLIT, dotDmg[1] / 5, false, 2500, 21)
