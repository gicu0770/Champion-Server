...
local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[110].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[110].manaCost,
  spellId = 110,
  range = GLOBAL_SPELL_COOLDOWNS[110].range,
  aggressive = true,
  forwardCast = true,
  cooldown = GLOBAL_SPELL_COOLDOWNS[110].cooldown,
  type = COMBAT_ENERGYDAMAGE,
  targets = GLOBAL_SPELL_COOLDOWNS[110].split,
...
local function onCastSpell(player, item, getInfoOnly, force, mousePos)
  ...
  if not target then
    ...
    if math.random(100) <= 20 then -- 25% chance to poison
      target:startDOT(player, SHOCK, 0, false, 5000)
    end
  end
  ...
  if colleftInfo[player:getId()].attributesItems[178] then
      local extraTargets = getClosestTargets(player, target, player:getPosition(), CONFIG_SUP.range, US_ENCHANTMENTS[178].subvalue, true)
