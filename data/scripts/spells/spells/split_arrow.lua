...
local CONFIG = {
  spellName = GLOBAL_SPELL_COOLDOWNS[82].name,
  level = 1,
  magLevel = 0,
  precentCost = true,
  manaCost = GLOBAL_SPELL_COOLDOWNS[82].manaCost,
  spellId = 82,
  range = GLOBAL_SPELL_COOLDOWNS[82].range,
  aggressive = true,
  forwardCast = true,
  cooldown = GLOBAL_SPELL_COOLDOWNS[82].cooldown,
  type = COMBAT_PHYSICALDAMAGE,
  projectile = GLOBAL_SPELL_COOLDOWNS[82].split,
...
local function onCastSpell(player, item, getInfoOnly, force, mousePos)
  ...
  if spellExecuteCombat(player, combat, CONFIG, CONFIG_SUP, item, variant) then
    if CONFIG_SUP.projectile >= 1 then
      local extraTargets = getClosestTargets(player, target, player:getPosition(), CONFIG_SUP.range, CONFIG_SUP.projectile, true)
