local removePos = Position(537,1744,12)

local function resetTRIP(p)
    doTransformItem(getTileItemById(p, 9826).uid, 9825)
    doCreateItem(9485, 1, removePos) --Item's ID AND Position that you want to remove
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
        ...
        elseif item.itemid == 9825 then
            doRemoveItem (getTileItemById(removePos, 9485).uid) --Same item's ID AND Position that you want to remove
