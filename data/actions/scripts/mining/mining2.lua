...
function onUse(cid, item, fromPosition, target, toPosition, isHotkey)
  ...
  if player:getLevel() < 200 then
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need Level 200+ to use this pickaxe!")
  return false
  end
  ...
  if math.random(1,100) <= chance then
	...
	--		local clone = Game.createMonster("Stone Golem", player:getPosition(), true, true)
	--		if clone then
