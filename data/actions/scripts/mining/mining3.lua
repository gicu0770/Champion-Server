...
function onUse(cid, item, fromPosition, target, toPosition, isHotkey)
  ...
  if player:getLevel() < 300 then
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need Level 300+ to use this pickaxe!")
  return false
  end
  ...
  if math.random(1,100) <= chance then
	...
	--		local clone = Game.createMonster("War Golem", player:getPosition(), true, true)
	--		if clone then
